...
class HostPostItem extends Component{
    render(){
        ...
        if(typeof published_date !== "undefined"){
            date = published_date.replace("T", " ").replace("Z","").slice(0,16)
        }
        return(
            <div className={css(d.flex, fled.r, w.w_100, h.lg,pad.esm, m.mg_b_lg)}>
                <img src={images && images.length? images[0]: image.post_img} className={css(w.lg, fle.flex_1)} style={{objectFit:"cover"}}></img>
                ...
                (<div className={css(ff.Roboto,fw.w500,lets._esm,clr.white,text.u,fs.esm)}>{date}</div>): (<div></div>)
                }
                </div>
            </div>
        )
